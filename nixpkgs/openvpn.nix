# Copypasta from nixpkgs/nixos/modules/services/networking/openvpn.nix @ cb14135
{ config, lib, pkgs, ... }:
...
let
  ...
  makeOpenVPNJob = cfg: name:
    let
      ...
      path = makeBinPath (getAttr "openvpn-${name}" config.launchd.services).path;
    ...
    in {
      ...
      script = "${openvpn}/sbin/openvpn --suppress-timestamps --config ${configFile}";
